...
print(merged_df[['BMI', 'SedentaryMinutes', 'TotalSteps', 'Calories', 'total_sleep_minutes', 'Value']].isnull().sum())

# Drop rows with missing values in critical columns
merged_df = merged_df.dropna(subset=['total_sleep_minutes', 'Value'])
...
# Print calculated HRS and Framingham Risk Score for debugging
print("Calculated HRS and Framingham Risk Score:")
print(merged_df[['Id', 'ActivityDate', 'HRS', 'FraminghamRiskScore']].head(20))
...
for id, group in grouped:
    ...
    plt.plot(group['ActivityDate'], group['HRS'], marker='o', color='blue', label='Health Risk Score (HRS)')
    plt.xlabel('Date')
    plt.ylabel('Health Risk Score (HRS)')
    plt.title(f'Health Risk Score Over Time for ID: {id}')
    plt.legend()
    plt.grid()
    plt.show()
