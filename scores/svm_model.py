...
def train_svm():
    """
    Train and evaluate an SVM model for health status prediction using a linear kernel.
    """
    # Load preprocessed data from CSV files
    data = load_preprocessed_data()
    
    # Create SVM model with a linear kernel for multi-class classification
    svm_model = SVC(kernel='linear', probability=True, random_state=42)
    
    # Perform cross-validation
    cv_scores = cross_val_score(svm_model, data['X_train_scaled'], data['y_train'], cv=5)
    print("\nCross-validation scores:", cv_scores)
    print("Average CV score:", cv_scores.mean())
    print("CV score standard deviation:", cv_scores.std())
    
    # Train the final model
    svm_model.fit(data['X_train_scaled'], data['y_train'])
    
    # Make predictions
    y_pred = svm_model.predict(data['X_test_scaled'])
    ...
    # Feature importance for SVM with linear kernel: use the average absolute coefficients
    if svm_model.kernel == 'linear':
        coefficients = np.mean(np.abs(svm_model.coef_), axis=0)
    ...
    return svm_model, data['imputer'], data['scaler']
...
if __name__ == "__main__":
    # Train the SVM model and get the trained model and preprocessing objects
    model, imputer, scaler = train_svm()
