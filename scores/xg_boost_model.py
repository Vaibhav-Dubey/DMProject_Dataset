...
def train_xgboost():
    """
    Train and evaluate an XGBoost model for health status prediction.
    """
    # Load preprocessed data from CSV files
    data = load_preprocessed_data()
    
    # Create the XGBoost classifier
    # Note: use_label_encoder=False avoids a warning and eval_metric is set for multi-class classification.
    xgb_model = XGBClassifier(random_state=42, use_label_encoder=False, eval_metric='mlogloss')
    
    # Perform cross-validation
    cv_scores = cross_val_score(xgb_model, data['X_train_scaled'], data['y_train'], cv=5)
    ...
    print(classification_report(data['y_test'], y_pred, 
                                target_names=['Sedentary', 'Low Active', 'Somewhat Active', 'Active', 'Highly Active']))
    ...
    return xgb_model, data['imputer'], data['scaler']
